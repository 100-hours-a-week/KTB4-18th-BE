...
| P2 | 잠금 단계 상한 | 실제 수치 진단에서 47번째 잠금(약 3.518×10^14분)부터 `Instant`가 미래 시각을 표현하지 못해 `DateTimeException`이 먼저 발생했다. 따라서 64번째 잠금의 비트 시프트 순환보다 앞서 실패한다. `lockoutCount` 또는 최대 잠금 시간을 상한 처리하는 후속 개선이 안전하다. |
| P3 | 이메일 대소문자 데이터 정합성 | 조회 전 이메일을 소문자로 바르므로 가입 과정도 동일 정규화를 보장해야 한다. 그렇지 않으면 대문자를 보존한 기존 사용자 데이터를 찾지 못할 수 있다. |
| 해결 완료 | 타이밍 기반 계정 추측 완화 | 미존재·탈퇴 사용자도 고정 더미 BCrypt 해시와 `PasswordEncoder.matches()`를 수행한다. 응답뿐 아니라 비밀번호 비교 비용도 실제 계정 실패 경로와 가깝게 맞췄다. |
