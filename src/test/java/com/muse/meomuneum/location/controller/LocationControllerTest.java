...
class LocationControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new LocationController(service))
                .setControllerAdvice(new LocationExceptionHandler(), new GlobalExceptionHandler())
                .build();
    }

    @Test
    void returnsSnakeCaseLocationResolutionWithoutOriginalCoordinates() throws Exception {
        LocationResolveResponse response = new LocationResolveResponse(
                null,
    ...
    void returnsSpecifiedErrorForInsufficientAccuracy() throws Exception {
        when(service.resolve(any(), any()))
                .thenThrow(new LocationException(LocationErrorCode.INVALID_COORDINATES));
        ...
        mockMvc.perform(post("/api/v1/locations/resolve")
                ...
                .andExpect(jsonPath("$.message")
                        .value("invalid coordinates or location accuracy insufficient"))
