...
class RecommendationApiTests {
    ...
    @MockitoBean RecommendationProvider provider;
    MockMvc mvc;

    @BeforeEach void setUp() {
        mvc = MockMvcBuilders.webAppContextSetup(context).apply(springSecurity()).build();
        when(provider.recommend(anyList())).thenReturn(java.util.List.of(
                new TrackData("ITUNES", "1", "song 1", "artist", null, null),
    ...
    @Test void savesFiveSongsAndPromptAndGuestCanRetrieve() throws Exception {
        var session = new MockHttpSession();
        mvc.perform(post("/api/v1/recommendations").session(session).contentType("application/json").content(body("비 오는 밤")))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.data.items.length()").value(5))
        ...
        assertEquals("비 오는 밤", jdbc.queryForObject(
    ...
    @Test void returnsPartialResultsAndPassesPreviousPromptsToProvider() throws Exception {
        when(provider.recommend(anyList())).thenReturn(java.util.List.of(
                new TrackData("ITUNES", "partial-1", "song", "artist", null, null)));
        var session = new MockHttpSession();
        mvc.perform(post("/api/v1/recommendations").session(session)
                .contentType("application/json").content(body("비 오는 밤")))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.data.items.length()").value(1));
        mvc.perform(post("/api/v1/recommendations").session(session)
                .contentType("application/json").content(body("드라이브").replace("TEXT", "VOICE")))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.data.items.length()").value(1));
    ...
    void transactionRollsBackWhenOneTrackCannotBeStored() {
        ...
        assertThrows(RuntimeException.class, () -> template.execute(status -> new RecommendationService(
                broken, repository, new SimpleMeterRegistry())
