...
class ChatRoomControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ChatRoomController(service))
                .setControllerAdvice(new ChatRoomExceptionHandler())
                .build();
    }

    @Test
    void returnsThePreProvisionedRoomForARegion() throws Exception {
        when(service.findByRegion(25L)).thenReturn(
                new ChatRoomResponse(700L, 25L, "성남시 분당구", 25, "ACTIVE")
        );

        mockMvc.perform(get("/api/v1/regions/25/chat-room"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("chat room retrieved"))
                .andExpect(jsonPath("$.data.room_id").value(700L))
                .andExpect(jsonPath("$.data.region_id").value(25L))
    ...
    void returnsCreatedForTheFirstJoin() throws Exception {
        ...
        when(service.join(7L, 700L, "loc_token"))
                .thenReturn(new ChatRoomJoinResult(membership, true));

        mockMvc.perform(post("/api/v1/chat-rooms/700/members")
                        .principal(new UsernamePasswordAuthenticationToken(7L, null))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"location_resolution_token\":\"loc_token\"}"))
    ...
    void returnsOkAndTheExistingMembershipForAnIdempotentRetry() throws Exception {
        ChatRoomMembershipResponse membership = membership();
        when(service.join(7L, 700L, "loc_token"))
                .thenReturn(new ChatRoomJoinResult(membership, false));
